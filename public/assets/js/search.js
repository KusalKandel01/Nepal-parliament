  ...
  function renderSuggestions(query) {
    ...
    suggestBox.innerHTML = matches.map(m => `
      <a class="suggest-item" href="member.html?id=${m.id}">
        <span class="suggest-avatar" style="background:${APP.partyColorVar(m.p)}" translate="no">${(m.n||"?").replace(/^मा\.\s*/,"").charAt(0)}</span>
    ...
    `).join("") + `<button type="button" class="suggest-all" data-q="${APP.escapeHtml(query)}">${APP.t("suggest_view_all").replace("{q}", APP.escapeHtml(query))}</button>`;
    suggestBox.hidden = false;
