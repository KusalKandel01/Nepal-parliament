...
(async function () {
  ...
  document.title = `${displayName} | ${siteSuffix}`;
  document.getElementById("pageTitle").textContent = document.title;
  APP.renderSubnav(displayName, "directory.html", "nav_directory");
  const partyName = APP.partyLabelFor(m.party_code, m.party_ne) || "";
  const districtName = APP.districtLabelFor(m.district) || "";
  const houseName = m.house === "HoR" ? APP.t("house_hor") : APP.t("house_na");
  const desc = lang === "en"
    ? `${displayName}, ${partyName}, ${districtName}. ${houseName} member contact: phone, email.`
    : `${displayName}, ${partyName}, ${districtName}। ${houseName} सदस्यको सम्पर्क: फोन, इमेल।`;
  ...
  document.getElementById("pageCanonical").setAttribute("href", `https://nepal-parliament-directory.vercel.app/${lang}/member.html?id=${encodeURIComponent(m.id)}`);
    ...
    <div class="profile-card party-accent" data-party-color="${color}">
      <div class="profile-top">
        ...
        <div>
          ...
          <h1 class="profile-name">${APP.escapeHtml(primaryName)}</h1>
      ...
      <div class="profile-details">
        ...
        <div class="detail-item">
          ...
          <div class="v">${APP.escapeHtml(APP.districtLabelFor(m.district) || APP.t("district_unspecified"))}</div>
        </div>
        <div class="detail-item">
