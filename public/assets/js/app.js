...
const APP = (() => {
  ...
  // A handful of members.json district values are stored only in Devanagari
  // (province names for National Assembly members, plus 2 district names).
  // Unlike guessing an arbitrary district's spelling, this is a small, fixed,
  // unambiguous set of real Nepali place names, so mapping it is safe.
  const DISTRICT_LABEL_EN = {
    "कोशी": "Koshi Province", "मधेश": "Madhesh Province", "बागमती": "Bagmati Province",
    "गण्डकी": "Gandaki Province", "लुम्बिनी": "Lumbini Province", "कर्णाली": "Karnali Province",
    "सुदूरपश्चिम": "Sudurpashchim Province",
    "काठमाडौं-९": "Kathmandu-9", "धनुषा": "Dhanusa",
  };
  // Standard Devanagari spellings for all 77 districts, since members.json
  // stores district names in English script for almost every row. On
  // Nepali-language pages this is used so the district field never shows
  // untranslated English text.
  const DISTRICT_LABEL_NE = {
    "Achham": "अछाम", "Arghakhanchi": "अर्घाखाँची", "Baglung": "बाग्लुङ",
    "Baitadi": "बैतडी", "Bajhang": "बझाङ", "Bajura": "बाजुरा",
    "Banke": "बाँके", "Bara": "बारा", "Bardiya": "बर्दिया",
    "Bhaktapur": "भक्तपुर", "Bhojpur": "भोजपुर", "Chitwan": "चितवन",
    "Dadeldhura": "डडेलधुरा", "Dailekh": "दैलेख", "Dang": "दाङ",
    "Darchula": "दार्चुला", "Dhading": "धादिङ", "Dhankuta": "धनकुटा",
    "Dhanusa": "धनुषा", "Dolakha": "दोलखा", "Dolpa": "डोल्पा",
    "Doti": "डोटी", "Gorkha": "गोरखा", "Gulmi": "गुल्मी",
    "Humla": "हुम्ला", "Ilam": "इलाम", "Jajarkot": "जाजरकोट",
    "Jhapa": "झापा", "Jumla": "जुम्ला", "Kailali": "कैलाली",
    "Kalikot": "कालिकोट", "Kanchanpur": "कञ्चनपुर", "Kapilbastu": "कपिलवस्तु",
    "Kaski": "कास्की", "Kathmandu": "काठमाडौं", "Kavrepalanchowk": "काभ्रेपलाञ्चोक",
    "Khotang": "खोटाङ", "Lalitpur": "ललितपुर", "Lamjung": "लमजुङ",
    "Mahottari": "महोत्तरी", "Makawanpur": "मकवानपुर", "Manang": "मनाङ",
    "Morang": "मोरङ", "Mugu": "मुगु", "Mustang": "मुस्ताङ",
    "Myagdi": "म्याग्दी", "Nawalparasi": "नवलपरासी", "Nawalparasi (East of Bardaghat Susta)": "नवलपरासी (पूर्व)",
    "Nuwakot": "नुवाकोट", "Okhaldhunga": "ओखलढुंगा", "Palpa": "पाल्पा",
    "Panchthar": "पाँचथर", "Parbat": "पर्वत", "Parsa": "पर्सा",
    "Pyuthan": "प्युठान", "Ramechhap": "रामेछाप", "Rasuwa": "रसुवा",
    "Rautahat": "रौतहट", "Rolpa": "रोल्पा", "Rukum East": "रुकुम (पूर्व)",
    "Rukum West": "रुकुम (पश्चिम)", "Rupandehi": "रुपन्देही", "Salyan": "सल्यान",
    "Sankhubasabha": "संखुवासभा", "Saptari": "सप्तरी", "Sarlahi": "सर्लाही",
    "Sindhuli": "सिन्धुली", "Sindhupalchowk": "सिन्धुपाल्चोक", "Siraha": "सिराहा",
    "Solukhumbu": "सोलुखुम्बु", "Sunsari": "सुनसरी", "Surkhet": "सुर्खेत",
    "Syangja": "स्याङ्जा", "Tanahun": "तनहुँ", "Taplejung": "ताप्लेजुङ",
    "Terhathum": "तेह्रथुम", "Udayapur": "उदयपुर",
  };
  function districtLabelFor(district) {
    if (!district) return district;
    if (getLang() === "en") return DISTRICT_LABEL_EN[district] || district;
  ...
  function siblingLangUrl(targetLang) {
    ...
    if (path.includes("/ne/")) return path.replace("/ne/", `/${targetLang}/`) + location.search;
    if (path.includes("/en/")) return path.replace("/en/", `/${targetLang}/`) + location.search;
  ...
  function initLangToggle() {
    ...
    mount.innerHTML = `
      <div class="lang-toggle" role="group" aria-label="${t("lang_toggle_label")}">
        <a href="${siblingLangUrl("ne")}" data-lang="ne" class="${lang === "ne" ? "active" : ""}" aria-current="${lang === "ne" ? "page" : "false"}">ने</a>
        <a href="${siblingLangUrl("en")}" data-lang="en" class="${lang === "en" ? "active" : ""}" aria-current="${lang === "en" ? "page" : "false"}">EN</a>
      </div>`;
    mount.querySelectorAll("a").forEach(link => {
      link.addEventListener("click", () => {
        // Remember the choice for next time the person lands on the root
        // redirect page; the click itself is a normal navigation to the
        // sibling-language page, not a same-page reload.
        window.I18N_ENGINE.setLang(link.dataset.lang);
      });
    });
  }
