...
(async function () {
  ...
  function personMarkup(nameNe, nameEn, isTransliterated) {
    if (!nameNe) return "N/A";
    const key = nameNe.replace(/^श्री\s*|^मा\.\s*|^डा\.\s*/, "").trim();
    const id = memberIndex[key];
    const label = APP.escapeHtml(lang === "en" ? (nameEn || nameNe) : nameNe);
    const inner = id ? `<a href="member.html?id=${id}">${label}</a>` : label;
    const unverifiedNote = (lang === "en" && isTransliterated)
    ...
    list.innerHTML = items.map(c => {
      ...
      <div class="committee-card">
        ...
        <div class="committee-people">
          <div><div class="role">${APP.t("committee_chair_role")}</div>${personMarkup(c.chair_ne, c.chair_en, c.chair_en_transliterated)} ${c.chair_office ? `· <a href="tel:${c.chair_office.replace(/\D/g,'')}" translate="no">${c.chair_office}</a>` : ""}</div>
          ${c.secretary_ne ? `<div><div class="role">${APP.t("committee_secretary_role")}</div>${personMarkup(c.secretary_ne, c.secretary_en, c.secretary_en_transliterated)} ${c.secretary_office ? `· <a href="tel:${c.secretary_office.replace(/\D/g,'')}" translate="no">${c.secretary_office}</a>` : ""}</div>` : ""}
